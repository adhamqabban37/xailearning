...
describe("Enhanced YouTube URL utilities", () => {
  describe("extractYouTubeId", () => {
    it("extracts from standard watch URLs", () => {
      ...
      expect(extractYouTubeId("https://youtube.com/watch?v=abc123&t=42")).toBe(
        "abc123"
      );
    });

    it("extracts from youtu.be share links", () => {
      expect(extractYouTubeId("https://youtu.be/jNQXAC9IVRw")).toBe(
        "jNQXAC9IVRw"
      );
      expect(extractYouTubeId("https://youtu.be/xyz789?si=tracking123")).toBe(
        "xyz789"
      );
    });
    ...
    it("extracts from Shorts URLs", () => {
      expect(
        extractYouTubeId("https://www.youtube.com/shorts/2vj37YE4hTA")
      ).toBe("2vj37YE4hTA");
      expect(
        extractYouTubeId("https://youtube.com/shorts/SHORT_ID?feature=share")
      ).toBe("SHORT_ID");
    });
    ...
    it("extracts from embed URLs", () => {
      ...
      expect(
        extractYouTubeId("https://www.youtube-nocookie.com/embed/NOCOOKIE_ID")
      ).toBe("NOCOOKIE_ID");
    });

    it("extracts from legacy /v/ URLs", () => {
      expect(extractYouTubeId("https://www.youtube.com/v/LEGACY_ID")).toBe(
        "LEGACY_ID"
      );
    });
    ...
    it("handles URLs with tracking params", () => {
      const urlWithTracking =
        "https://www.youtube.com/watch?v=TEST123&utm_source=share&si=abc&list=playlist";
    ...
    it("handles mobile URLs", () => {
      expect(extractYouTubeId("https://m.youtube.com/watch?v=MOBILE_ID")).toBe(
        "MOBILE_ID"
      );
    });
  });
  ...
  describe("toEmbedUrl", () => {
    it("generates clean embed URLs with safe params", () => {
      ...
      expect(url).toBe(
        "https://www.youtube.com/embed/TEST_VIDEO_ID?rel=0&modestbranding=1&controls=1"
      );
    });
