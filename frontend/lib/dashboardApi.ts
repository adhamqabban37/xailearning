    ...
    try {
      ...
      mockModules.forEach((module) => {
        ...
        moduleProgressMap[module.id] = {
          ...
          completionPercentage: progress?.module_completed
            ? 100
            : (progress?.actions_completed?.length || 0) > 0
            ? 50
            : 0,
      ...
      return {
        ...
        estimatedTimeRemaining: Math.max(
          0,
          (modules.length - completedModules) * 120
        ),
  ...
  updateModuleProgress: async (
    ...
    try {
      const updateData = {
        ...
        minutes: progressData.timeSpent,
        day_key: progressData.dayKey,
        actions_done: progressData.actions,
  ...
  batchUpdateModules: async (
    ...
    try {
      const results = await Promise.allSettled(
        updates.map((update) =>
          dashboardApi.toggleModuleCompletion(
            courseId,
            update.moduleId,
            update.completed
          )
