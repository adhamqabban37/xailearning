...
// Use UploadResponse type from lib/api
...
export default function FileUpload() {
  ...
  const validateFile = (file: File): string | null => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      )} MB. Current size: ${Math.round(file.size / (1024 * 1024))} MB.`;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const lessonCount =
        result.course?.lessons?.length || result.course?.modules?.length || 0;
      setSuccess(
        `File uploaded successfully! Parsed ${lessonCount} lessons/modules.`
      );
          ...
          <div className="mt-8 border-t pt-8">
            ...
            <div className="space-y-4">
              <h5 className="font-semibold text-gray-900">
                Modules ({uploadResult.course.modules.length})
              </h5>
                      ...
                      <p className="text-sm text-gray-700">
                        ...
                        {module.summary_notes.length > 1 && (
                          <span className="text-gray-500">
                            {" "}
                            +{module.summary_notes.length - 1} more notes
                          </span>
                        )}
                      </p>
                    </div>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {(uploadResult.course?.lessons?.length || 0).toString()}
                  </p>
                </div>
                <div>
                  <span className="text-gray-600">Modules:</span>
                  <p className="font-medium">
                    {(uploadResult.course?.modules?.length || 0).toString()}
                  </p>
                </div>
                <div>
