...
import { progressScenarios, sampleDashboardStats } from '../data/sampleProgressData'
...
export function ProgressDashboardDemo() {
  const [currentScenario, setCurrentScenario] = useState<ScenarioKey>('halfway')
  const [demoMode, setDemoMode] = useState(true)

  const currentData = progressScenarios[currentScenario]
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <div className="mt-2 flex items-center space-x-4 text-xs text-blue-600">
                    <span>{currentData.completedModules}/{currentData.totalModules} modules completed</span>
                    <span>{currentData.completionPercentage}% overall progress</span>
                    <span>{Math.floor(currentData.totalTimeSpent / 60)}h {currentData.totalTimeSpent % 60}m spent</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">
                <p>
                  🎯 <strong>Interactive Demo:</strong> Try clicking module completion toggles, 
                  switching scenarios, and exploring the progress visualizations.
                </p>
              </div>
